...
function(sbeUpdateUrlInContextFile contextFile name url isFound)
    file(READ ${contextFile} context)
    
    if("${context}" MATCHES "(Project|Package)([ \t]+${name}[^\n]+Url[ \t]+)[^ \t\n]+")
        string(REGEX REPLACE "(Project|Package)([ \t]+${name}[^\n]+Url[ \t]+)[^ \t\n]+" "\\1\\2${url}" context "${context}")
        file(WRITE ${contextFile} ${context})
        set(${isFound} yes PARENT_SCOPE)
    else()
        set(${isFound} no PARENT_SCOPE)
    endif()    
endfunction()